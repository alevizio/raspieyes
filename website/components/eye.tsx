...
interface EyeConfig {
  size: number;
  scleraColor: string;
  irisColors: [string, string, string]; // outer, mid, inner
  pupilColor: string;
...
export const Eye = ({
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, dt: number) => {
      ...
      const { size, irisRatio, pupilRatio, parallax, maxOffset } = config;
      const cx = size / 2;
      const cy = size / 2;
      ...
      anim.currentX += (targetX - anim.currentX) * factor;
      anim.currentY += (targetY - anim.currentY) * factor;
      ...
      anim.breatheTime += dt;
      const breathe =
        Math.sin(anim.breatheTime * 0.2 * TAU) * 0.03;
      ...
      scleraGrad.addColorStop(0.6, config.scleraColor);
      ...
      irisGrad.addColorStop(0, config.irisColors[2]);
      irisGrad.addColorStop(0.4, config.irisColors[1]);
      irisGrad.addColorStop(0.75, config.irisColors[0]);
      ...
      pupilGrad.addColorStop(0, config.pupilColor);
      pupilGrad.addColorStop(0.8, config.pupilColor);
      ...
      hlGrad.addColorStop(0, config.highlightColor);
